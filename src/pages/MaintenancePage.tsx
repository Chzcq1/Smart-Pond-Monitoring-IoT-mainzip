      ...
      <div className="max-w-[1400px] mx-auto w-full space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Sensor cleaning and calibration tracking across all facilities.</p>
          </div>
            ...
            {overdueCount > 0 && (
              <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl px-4 py-2 flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-rose-400 leading-none">{overdueCount} Overdue</span>
                  <span className="text-[10px] uppercase text-rose-500/70">Action Required</span>
                </div>
              </div>
            )}
            {dueSoonCount > 0 && (
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-2 flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-amber-400 leading-none">{dueSoonCount} Due Soon</span>
                  <span className="text-[10px] uppercase text-amber-500/70">Plan Ahead</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="bg-[#0f172a] border border-[#1e293b] rounded-xl p-4 flex flex-wrap gap-4 shrink-0">
          <div className="relative flex-1 min-w-[200px] max-w-md">
            ...
            <input 
              type="text" 
              placeholder="Search station or facility..." 
              value={searchTerm}
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-[#0f172a] sticky top-0 z-10 border-b border-[#1e293b] shadow-sm">
                <tr className="text-[10px] uppercase tracking-wider text-slate-500">
                  <th className="px-6 py-4 font-semibold">Station Info</th>
                  <th className="px-6 py-4 font-semibold">Status</th>
                  <th className="px-6 py-4 font-semibold">Sensor Cleaning</th>
                  <th className="px-6 py-4 font-semibold">System Calibration</th>
                  <th className="px-6 py-4 font-semibold text-right">Time Remaining</th>
                </tr>
              </thead>
                    ...
                    <tr key={station.id} className="hover:bg-[#1e293b]/30 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col text-xs font-mono-data">
                          <span className="text-slate-500 mb-0.5">Last: {format(station.maintenance.lastCleaning, 'dd MMM yyyy')}</span>
                          <span className="text-slate-300 font-semibold">Next: {format(station.maintenance.nextCleaning, 'dd MMM yyyy')}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col text-xs font-mono-data">
                          <span className="text-slate-500 mb-0.5">Last: {format(station.maintenance.lastCalibration, 'dd MMM yyyy')}</span>
                          <span className="text-slate-300 font-semibold">Next: {format(station.maintenance.nextCalibration, 'dd MMM yyyy')}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex flex-col items-end">
                          ...
                          <span className="text-[10px] uppercase tracking-wider text-slate-500 mt-1">Days Left</span>
                        </div>
                      </td>
                    </tr>
                  ))
...
function StatusPill({ status }: { status: 'ok' | 'due-soon' | 'overdue' }) {
  if (status === 'ok') {
    return <span className="inline-flex items-center px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">Optimal</span>;
  }
  if (status === 'due-soon') {
    return <span className="inline-flex items-center px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-amber-500/10 text-amber-400 border border-amber-500/20">Due Soon</span>;
  }
  return <span className="inline-flex items-center px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-rose-500/10 text-rose-400 border border-rose-500/20 animate-pulse">Overdue</span>;
}
